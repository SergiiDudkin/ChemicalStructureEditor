...
export function vecLen(xy) { // Find length of vector
	return Math.sqrt(sqVecLen(xy));
}

export function findDist(xy0, xy1) { // Find distance between two points
...
export function vecDif([x0, y0], [x1, y1]) { // Subtract vectors
	return [x1 - x0, y1 - y0];
}

export function vecMul([x, y], factor) { // Multiply vector by scalar value
	return [x * factor, y * factor];
}

export function vecDiv([x, y], divisor) { // Divide vector by scalar value
...
export function cosVec(xy0, xy1) { // Find cos between two vectors
	return clampSinCos(vecDotProd(xy0, xy1) / (vecLen(xy0) * vecLen(xy1)));
}

export function sinVec(xy0, xy1) { // Find sin between two vectors
...
export function angleBisector(xy0, xy1) { // Not normalized, no direction control
...
export function polygonEdgeCtrDist(angle, side_len) {
	return side_len * Math.tan(angle / 2) / 2;
}

export function polygonVertexCtrDist(angle, side_len) {
